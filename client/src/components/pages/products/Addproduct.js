...
class AddProduct extends Component {
    ...
    if (color) {
      return color.map((color) => (
        <div className="row color-row" key={color.id || color._id}>
          ...
          <div className="col-md-12">{this.getSizeboxes(color.id)}</div>
          <div className="row">
    ...
    return color_obj[0].sizes.map((size) => (
      <div className="sizes_box" key={size.id || size._id}>
        <div className="row">
          <div
            className="left"
            style={{ width: "95%", paddingLeft: "40px", paddingRight: "10px" }}
          >
            <input
              type="text"
              name="size"
  ...
  handleChange = (e, color_id = "", size_id = "") => {
    ...
    const colorIndex = color.findIndex((color) => color.id == color_id);
    if (size_id != "") {
      // get all sizes
      let { sizes } = color_obj;
